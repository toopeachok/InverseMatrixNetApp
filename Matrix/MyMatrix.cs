...
namespace Matrix
{
  public class MyMatrix
  {
    ...
    public static string MatrixToRawString(double[][] m)
    {
      String matrixString = "";
    ...
    public static double[][] getMatrixFromRawString(string matrixString, char separator)
    {

      double[][] m = MatrixCreate(2, 2);

      try
      {

        string[] matrixStringValues = matrixString.Split(separator);

        int dimension = (int)Math.Sqrt(matrixStringValues.Length);

        m = MatrixCreate(dimension, dimension);

        foreach (string value in matrixStringValues)
        {
          for (int i = 0; i < dimension; ++i)
          {
            ...
            {
              m[i][j] = Convert.ToDouble(matrixStringValues[(i * dimension) + j]);
            }
      ...
      {
        Console.WriteLine("Incorrect matrix data");
      }
